...
void hubEncoder(){
  if(firstPass or backToZero){
    ...
    for(iWheel = 0; iWheel <= countWheel; iWheel++){
      ...
      //Serial.println(deltaTime[iWheel]);
    }
...
void loop() {
  ...
  if (millis() - beginningTic > 1000 / Hz){
    ...
    analogWrite(vrPin,returnVariablesWheel.speedCheck);
    digitalWrite(zfPin,returnVariablesWheel.forwardBackward);
    Serial.print("speedCheck: ");
    Serial.println(returnVariablesWheel.speedCheck);
